...
>
> Fortress (control) -> Fracture (restaurant moment) -> Flight (impulsive bus) -> Liminal Night (difficult ride + unreconciled self) -> Dawn (first external alignment of inner shift) -> Exploration (mirrors, portals, strangers, findings) -> Integration (Return to Oxford with a reconfigured definition of balance and self) -> Coda (“the fortress has windows”).
